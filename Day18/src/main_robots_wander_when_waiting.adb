...
procedure Main is
   ...
   subtype Robot_Range is Positive range 1 .. Max_Robots  ;

   Vault: Vault_Map;
   Start_Position: array ( Robot_Range ) of Position;
   ...
   type Robot_Actives is array ( Robot_Range ) of Boolean;

   type Path_Data is record
      Cost: Natural; -- distance traveled on the path
      Paths: Path_Array; -- positions traveled on the path
      Active: Robot_Actives;
   ...
   function "="(First, Second: State_Cache) return Boolean is
     ...
     (
      First.Where = Second.Where and then First.Which = Second.Which and then
   ...
   begin
      ...
      -- at this point where is always the same
      for R in Robot_Range loop
         if First.Which(R) /= Second.Which(R) then
            return ( not First.Which(R) );
         end if;
      end loop;
      -- at this point where and which are the same
   ...
   is
      ...
      S: State_Cache := ( (
                          ...
                          Data.Active,
                          Data.Keys_Held ); -- state of this position
      ...
      then
         ...
         if Good_To_Add(S, Length) then
            ...
            begin
               ...
               --  Put("enqueuing "); Put(New_Path); New_Line;
               Paths_To_Consider.Enqueue( (
                                          Length,
                                          ( (
                                           for R in Robot_Range =>
                                             (
                                              if R = Robot then New_Path
                                              else Data.Paths(R)
                                             )
                                          ) ),
                                          Data.Active,
         ...
         begin
            ...
            while C /= No_RM_Element loop
               if Vault(Key(C).X, Key(C).Y).Feature = Key then
                  Keys(Vault(Key(C).X, Key(C).Y).Which_Key) := True;
               end if;
   ...
   procedure Setup_Search(Active: Robot_Actives) is
   ...
   begin
      ...
      for R in Robot_Range loop
         ...
         if Active(R) then Determine_Keys_For(R); end if;
      end loop;
      Paths_To_Consider.Enqueue( ( 0, Paths, Active, K ) );
      ...
      while Natural(Paths_To_Consider.Current_Use) /= 0 loop
         ...
         begin
            ...
            for R in Robot_Range loop
               ...
               if Data.Active(R) then

                  --  Put("considering"); Put(Data.Paths(R)); New_Line;
                  --  Put("with keys");
                  --  for K in Key_Name loop if Data.Keys_Held(K) then Put(K); end if; end loop; New_Line;

                  Last := Data.Paths(R).Last_Element;
                  ...
                  then
                     ...
                     if (
                         for all K in Key_Name =>
                           ( if Robot_Goal(R)(K) then Data.Keys_Held(K) )
                        )
                     then
                        Data.Active(R) := False;
                        if (
                            for all Robot in Robot_Range =>
                               not Data.Active(Robot)
                           )
                        then
                           return Data;
                        end if;
                  ...
                  if not Data.Active(R) then
                     ...
                     then
                        C := Graph(Last).First;
                        while C /= No_RM_Element loop
                           Add_If_Good( Data, ( Last, Key(C) ), Element(C), R );
                           Next(C);
                        end loop;
                     end if;
                  end if;
...
begin
   ...
   Start_Position(4) := ( @.X + 1, @.Y + 1 );
   Setup_Search( ( others => True ) );
